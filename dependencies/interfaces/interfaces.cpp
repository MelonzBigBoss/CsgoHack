...
bool interfaces::initialize() {
	...
	physics_props = get_interface<i_physics_props, interface_type::index>("vphysics.dll", "VPhysicsSurfaceProps001");
	mdl_cache = get_interface<i_mdl_cache, interface_type::index>("datacache.dll", "MDLCache004");  
	network_container = get_interface<i_network_container, interface_type::index>("engine.dll", "VEngineClientStringTable001");
	/*custom interfaces*/
	...
	view_render = **reinterpret_cast<i_view_render***>(utilities::pattern_scan("client.dll", "8B 0D ? ? ? ? FF 75 0C 8B 45 08") + 0x2);
	view_render_beam = *reinterpret_cast<i_view_render_beam**>(utilities::pattern_scan("client.dll", "B9 ? ? ? ? A1 ? ? ? ? FF 10 A1 ? ? ? ? B9") + 0x1);
	
	key_value_system = reinterpret_cast<KeyValuesSystemFn>(GetProcAddress(GetModuleHandle("vstdlib.dll"), "KeyValuesSystem"))();
	//key_value_system = (KeyValuesSystemFn)GetProcAddress(GetModuleHandle("vstdlib.dll"), XorStr("KeyValuesSystem"));
